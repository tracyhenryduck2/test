...
<html>    
  <head>  
	...
	<script type="text/javascript">    
		function add(){      
			Dialog.open({Title:"新增", Width:600, Height:360, URL:"<%=path%>/supermarket/MarketVedio!toAddMarketVedio.action?marketVedioBean.marketId=${marketVedioBean.marketId}"});   
		}      
		function mod(){      
			...
			Dialog.open({Title:"编辑", Width:600, Height:360, URL:"<%=path%>/supermarket/MarketVedio!toAddMarketVedio.action?oper=1&marketVedioBean.id="+id});
		}      
		function detail(){   
			var id = getCheckRowValue();     
			if(id==null) {     
			...
			Dialog.open({Title:"详情", Width:500, Height:360, URL:"<%=path%>/supermarket/MarketVedio!toAddMarketVedio.action?read=1&oper=1&marketVedioBean.id="+id});    
		}      
		function del(){      
			if(!isCheckAny("idArr")){      
