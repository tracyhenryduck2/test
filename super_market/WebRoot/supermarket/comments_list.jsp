...
<html>    
  <head>  
	...
	<script type="text/javascript">    
		function add(){      
			Dialog.open({Title:"新增", Width:600, Height:360, URL:"<%=path%>/supermarket/Comments!toAddComments.action"});   
		}      
		function mod(){      
			...
			Dialog.open({Title:"编辑", Width:600, Height:360, URL:"<%=path%>/supermarket/Comments!toAddComments.action?oper=1&commentsBean.id="+id});
		}      
		function detail(){   
			var id = getCheckRowValue();     
			if(id==null) {     
			...
			Dialog.open({Title:"详情", Width:500, Height:360, URL:"<%=path%>/supermarket/Comments!toAddComments.action?read=1&oper=1&commentsBean.id="+id});    
		}      
		function del(){      
			if(!isCheckAny("idArr")){      
			...
			Dialog.confirm("确定删除吗", function(){     
				...
				form1.action = "<%=path%>/supermarket/Comments!delCommentss.action";
				form1.target = "fram";
				form1.submit();  
		...
		function search() {  
			Dialog.open({Title:"查询", Width:650, Height:220, URL:"<%=path%>/supermarket/Comments!search.action",OnLoad:function(){
			     this.innerDoc.getElementById("member_id").value = $("#member_id").val();
			     this.innerDoc.getElementById("create_time").value = $("#create_time").val();
			     this.innerDoc.getElementById("content").value = $("#content").val();
			     this.innerDoc.getElementById("weight").value = $("#weight").val();
           }
			});    
		}      
  ...
  <body>  
  <GF:BodyCaption label="留言" ico="images/ico/user.gif"> 
  	<GF:ToolBar id="123">   
  		<!--  
    ...
    <form action="<%=path%>/supermarket/Comments!list.action" name="form1" id="form1" method="get">  
   
			<input type="hidden" name="commentsBean.memberId" id="memberId" value="${commentsBean.memberId}"/> 
			<input type="hidden" name="commentsBean.createTime" id="createTime" value="${commentsBean.createTime}"/> 
			<input type="hidden" name="commentsBean.content" id="content" value="${commentsBean.content}"/> 
			<input type="hidden" name="commentsBean.weight" id="weight" value="${commentsBean.weight}"/> 
			...
			<table id="table1" width="100%" height="100%"  border="0" cellspacing="0" cellpadding="0"> 
			   	 <thead>
			   		<tr>  
			   			...
			   			<th sortname="content" width="70%">内容</th>	   
			   			<th sortname="weight" width="10%">权重</th>	   
			   		</tr> 
			   	</thead>
			   	<tbody> 
